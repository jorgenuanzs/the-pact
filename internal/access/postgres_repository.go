...
func (r *PostgresRepository) CreateInvitation(
	...
	err = tx.QueryRow(ctx, `
		INSERT INTO identity.invitations (
			organization_id, project_id, email, role, organization_role, token_digest,
			expires_at, created_by_principal_id
		)
		VALUES ($1, $2, $3, $4, $5, $6, transaction_timestamp() + ($7 * interval '1 second'), $8)
		RETURNING id, project_id, email, role, organization_role, status, expires_at
	`, organizationID, projectID, input.Email, input.Role, input.OrganizationRole, digest[:],
